...
# -----------------------------------------------------------------------------
# 3. Model Fitting Wrapper (Updated for Flexible Structure)
# -----------------------------------------------------------------------------
fit_grm_models <- function(df_irt, ssq_items, phq_items, model_phq = 1, model_ssq = 1, model_joint = 1) {
  ...
  mod_phq <- mirt(phq_dat, model_phq, itemtype = 'graded', verbose = FALSE)
  ...
  mod_ssq <- mirt(ssq_dat, model_ssq, itemtype = 'graded', verbose = FALSE)
  ...
  mod_joint <- mirt(joint_dat, model_joint, itemtype = 'graded', verbose = FALSE)
...
# -----------------------------------------------------------------------------
check_group_invariance <- function(item_data, group_vec, group_name) {
  ...
  mod_mg <- multipleGroup(dat_grp[, -ncol(dat_grp)], group = dat_grp$Group, 
...
summarize_invariance <- function(res_obj, group_label) {
  ...
  # Robust check: If names are missing, provide defaults
  ...
  if (!is.null(focal_pars) && length(focal_pars) > 0) {
    if ("MEAN_1" %in% names(focal_pars)) {
      latent_mean <- focal_pars["MEAN_1"]
    } else if (length(focal_pars) >= 1) {
      latent_mean <- focal_pars[1]
    }
    
    if ("COV_11" %in% names(focal_pars)) {
      latent_var <- focal_pars["COV_11"]
    } else if (length(focal_pars) >= 2) {
      latent_var <- focal_pars[2]
    }
  }
...
run_invariance_workflow <- function(df_irt, ssq_items, phq_items, output_dir) {
  inv_file_path <- file.path(output_dir, "03_IRT_invariance_models.rds")
  run_checks <- TRUE
  
  if (file.exists(inv_file_path)) {
    message("\nExisting Invariance Models found.")
    if (interactive()) {
      if (tolower(substring(readline(prompt = "  Rerun Invariance checks (y/n)? "), 1, 1)) != "y") run_checks <- FALSE
    } else {
      run_checks <- FALSE
    }
  }
  ...
  if (run_checks) {
    ...
    res_sex <- check_group_invariance(clean_dat, df_irt$SEX[valid_joint], "SEX")
    res_age <- check_group_invariance(clean_dat, df_irt$AGEGRP[valid_joint], "AGEGRP")
    
    dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
    saveRDS(list(sex = res_sex, age = res_age), file = inv_file_path)
    message(paste("Invariance models saved."))
  } else {
    inv_results <- readRDS(inv_file_path)
    res_sex <- inv_results$sex
    res_age <- inv_results$age
    message("Invariance models loaded.")
  }
  
  # Generate Tables (Inside the function scope where variables exist)
...
perform_linking_and_scoring <- function(mod_joint, df_irt, ssq_items, phq_items) {
  ...
  # If model is multidimensional, fscores returns multiple cols.
  # df_irt$Theta_Harmonized will take the FIRST dimension (Primary/General).
  ...
  # Robust grid generation for multidimensional models
  n_dims <- mod_joint@Model$nfact
  t_seq <- seq(-4, 4, 0.01)
  
  if (n_dims == 1) {
    theta_grid_full <- matrix(t_seq)
  } else {
    # Vary Dim 1, hold others constant at 0
    theta_grid_full <- matrix(0, nrow=length(t_seq), ncol=n_dims)
    theta_grid_full[,1] <- t_seq
  }
  
  tcc_phq <- expected.test(mod_joint, Theta = theta_grid_full, which.items = phq_idx)
  ...
  metrics_df <- data.frame(
    ...
    Value = c(
      ...
      TP / (TP + FN),
      TN / (TN + FP),
      cohen.kappa(tbl)$kappa,
      expected.test(mod_joint, Theta = matrix(latent_cutoff, ncol=n_dims), which.items = (length(phq_items)+1):(length(phq_items)+length(ssq_items)))
    )
  )
  ...
  tbl_class <- gt(metrics_df) %>%
    ...
    tab_source_note(source_note = "Harmonized Classification based on Latent Theta threshold where expected PHQ-9 sum = 10.")
  
  tbl_conf <- gt(as.data.frame(tbl)) %>%
    tab_header(title = "Confusion Matrix") %>% cols_label(Freq = "Count")
  
  # 6. Plotting: Test Information
  # Use the same grid logic for plotting
  info_phq <- testinfo(mod_joint, theta_grid_full, which.items = phq_idx)
  info_ssq <- testinfo(mod_joint, theta_grid_full, which.items = (length(phq_items)+1):(length(phq_items)+length(ssq_items)))
  
  p_info <- ggplot(data.frame(Theta = rep(t_seq, 2), Information = c(info_phq, info_ssq), Scale = rep(c("PHQ-9", "SSQ-10"), each = length(t_seq))), 
...
# -----------------------------------------------------------------------------
# Creates a standalone tool to score NEW datasets using ONLY SSQ-10 items.

extract_scoring_engine <- function(mod_joint, ssq_items, cutoff_theta) {
  
  message("\nExtracting SSQ-10 Parameters for Scoring Engine...")
  
  # 1. Get all parameters from joint model
  pars <- mod2values(mod_joint)
  
  # 2. Filter for SSQ items only
  # Note: 'item' column in pars df matches item names
  pars_ssq <- pars[pars$item %in% ssq_items, ]
  
  # Detect model type (assuming 1D here, but storing logic)
  ...
  scoring_engine <- list(
    ...
    model_type = 'graded', # SSQ is binary/graded
    n_factors = nfact      # Store factor count for reconstruction
  )
...
score_new_cohort <- function(new_data, scoring_engine) {
  ...
  # 3. Build mirt object with FIXED parameters
  message("Estimating Theta for new cohort using fixed harmonization parameters...")
  
  # A. Generate a TEMPLATE parameter table valid for this new dataset
  ...
  # B. Map values from Scoring Engine to Template
  ...
  # 5. Classify
  # Use first factor (Theta 1) for classification
  ...
  return(data.frame(
    Theta_Harmonized = final_scores,
    Depression_Binary = final_class
  ))
}
...
run_irt_harmonization_pipeline <- function(phq_model = 1, ssq_model = 1, joint_model = 1) {
  ...
  # 2. Fit Models (With flexible structure)
  fit_res <- fit_grm_models(prep$data, prep$ssq_items, prep$phq_items,
                            model_phq = phq_model, model_ssq = ssq_model, model_joint = joint_model)
  
  # 3. Invariance Checks
  out_dir <- here("statistical_analysis/output/objects")
  inv_res <- run_invariance_workflow(prep$data, prep$ssq_items, prep$phq_items, out_dir)
  ...
  # 6. Save Final Object
  final_output <- list(
    input_structure = list(ssq_factors = ssq_model, phq_factors = phq_model, joint_factors = joint_model), # NEW: Metadata
...
# Execute
if (sys.nframe() == 0) { 
  # Example: Run with default 1-factor models
  # To use multidimensional models, change arguments:
  # res <- run_irt_harmonization_pipeline(joint_model = 2)
  
  res <- run_irt_harmonization_pipeline()
  
  # --- Example: Scoring New Data (Post-Validation) ---
  dreams_path <- here("../../_private_use/data_management/data/co-luminate/adam/dt_dreams_multi_ssq.RData")
  out_dir <- here("statistical_analysis/output/objects")
  
  if (file.exists(dreams_path)) {
    message(paste("\nLoading Dreams Cohort for scoring from:", dreams_path))
    load(dreams_path)
    ...
    if (exists("dt_dreams_multi_ssq")) {
      ...
      scored_dreams <- tryCatch({
        score_new_cohort(dt_dreams_multi_ssq, res$scoring_engine)
      }, error = function(e) {
      ...
      if (!is.null(scored_dreams)) {
        final_dreams_export <- scored_dreams
        if ("USUBJID" %in% names(dt_dreams_multi_ssq)) {
          final_dreams_export <- cbind(dt_dreams_multi_ssq[, "USUBJID", drop=FALSE], scored_dreams)
        }
        dreams_out_file <- file.path(out_dir, "Scored_Dreams_Cohort.rds")
        saveRDS(final_dreams_export, file = dreams_out_file)
    ...
    message("Dreams Cohort file not found (Skipping external scoring example).")
  }
}
