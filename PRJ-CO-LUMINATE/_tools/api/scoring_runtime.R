...
score_new_cohort <- function(new_data, scoring_engine) {
  ...
  if (length(missing) > 0) {
    stop("New data missing items: ", paste(missing, collapse = ", "))
  }
  ...
  if (sum(!keep_rows) > 0) {
    warning("Removed ", sum(!keep_rows), " rows with all-missing SSQ items.")
  }
...
.apply_cutoffs <- function(df, cutoff_table,
  ...
  if (!is.na(overall_cut)) {
    ...
    if (length(idx) > 0) {
      df$Depression_Harmonized[idx] <- ifelse(df$Theta_Harmonized[idx] >= overall_cut, 1L, 0L)
    }
  }
